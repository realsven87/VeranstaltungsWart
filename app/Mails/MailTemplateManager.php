...
class MailTemplateManager {
    ...
    public function register_template_cpt() {
        $labels = [
            ...
            'add_new_item'       => 'Neue E-Mail-Vorlage erstellen',
            'edit_item'          => 'E-Mail-Vorlage bearbeiten',
            'all_items'          => 'E-Mail-Vorlagen',
    ...
    public function render_placeholder_help($post) {
        ...
        if (isset($_GET['test_sent'])) {
            echo '<div class="notice notice-success inline"><p><strong>Test-Mail wurde versendet!</strong> Prüfe dein Postfach.</p></div>';
        }
        ?>
        <div class="notice notice-info inline" style="margin: 20px 0 0 0; border-left-color: #72aee6; background: #fff;">
            <p><strong>💡 Verfügbare Platzhalter:</strong><br>
            <code>{vorname}</code>, <code>{nachname}</code>, <code>{email}</code>, <code>{event_name}</code>, <code>{event_datum}</code>, <code>{event_zeit}</code>, <code>{event_adresse}</code>, <code>{event_plaetze}</code>, <code>{event_zeit}</code>, <code>{event_adresse}</code>, <code>{event_plaetze}</code>, <code>{storno_link}</code></p>
            <p style="font-size: 11px; color: #666; border-top: 1px dotted #ccc; padding-top: 8px;">
                Hinweis: Der <strong>Titel</strong> dieser Vorlage wird automatisch als <strong>E-Mail-Betreff</strong> verwendet.
            </p>
        </div>
        <?php
    }
    ...
    public function render_test_mail_box($post) {
        if ($post->post_status !== 'publish') {
            echo '<p class="description">Bitte die Vorlage erst <strong>veröffentlichen</strong>, um eine Test-Mail zu senden.</p>';
            return;
        }
        
        $user = wp_get_current_user();
        ?>
        <p style="margin-bottom: 10px;">Sende diese Vorlage mit Beispieldaten an deine Adresse:</p>
        <code style="display:block; margin-bottom: 15px;"><?php echo esc_html($user->user_email); ?></code>
        <a href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=vw_send_test_mail&template_id=' . $post->ID), 'vw_test_mail_nonce'); ?>" 
    ...
    public function handle_send_test_mail() {
        ...
        if (!$post) wp_die('Vorlage nicht gefunden.');
        ...
        $test_data = [
            ...
            'event_name'   => 'Demo-Veranstaltung am Sonntag',
            'event_datum'  => date_i18n(get_option('date_format')),
            'event_zeit'      => '18:00',
            'event_adresse'      => 'Musterstraße 1, 12345 Stadt',
            'event_plaetze'      => '2',
            'storno_link'  => '<a href="#">Anmeldung stornieren</a>'
        ];
    ...
    public function add_slug_column($columns) {
        ...
        foreach($columns as $key => $value) {
            ...
            if ($key === 'title') {
                $new_columns['slug'] = 'Technischer Name (Slug)';
            }
        }
        return $new_columns;
    }
    ...
    public static function get_rendered_mail($template_slug, $replacements) {
        ...
        if (!$template) {
            return [
                'subject' => '[' . get_bloginfo('name') . '] Benachrichtigung',
                'body'    => 'Systemfehler: E-Mail-Vorlage "' . esc_html($template_slug) . '" wurde nicht gefunden.'
            ];
        }
...
public function create_default_templates() {
        $defaults = [
            ...
            [
                'slug'    => 'ablehnung-info',
                'title'   => '[{event_name}] Anmeldung abgelehnt',
            ...
            [
                ...
                'title'   => '[{event_name}] Anmeldung auf der Warteliste',
            ...
            [
                'slug'    => 'freigabe-info',
                'title'   => '[{event_name}] Neue Anmeldung',
                'content' => "Eine neue Anmeldung für die Veranstaltung <strong>{event_name}</strong> am {event_datum} ist eingegangen und wartet auf Freigabe:\n\nName: <strong>{vorname} {nachname}</strong>\nE-Mail-Adresse: {email}\n\nBitte prüfe die Anmeldung.\n\nViele Grüße,\ndein VeranstaltungsWart"
            ]
        ];
